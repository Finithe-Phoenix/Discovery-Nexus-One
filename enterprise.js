...
(() => {
  ...
  function overview() {
    ...
    const eligiblePoints=points.filter(p=>state.operator==='all'||p.operators.includes(state.operator));
    const pending=S.requests(filter()).filter(r=>r.status==='Pendiente');
    const pendingAmount=pending.reduce((sum,r)=>sum+r.amount,0);
    const grouped=S.allowedOperators(state.role).map(o=>{const rows=m.confirmed.filter(t=>t.operator===o.id);return {...o,value:rows.reduce((sum,t)=>sum+t.amount,0),count:rows.length};}).filter(o=>state.operator==='all'||o.id===state.operator);
    ...
    return `<div class="kpi-grid">
      ...
      ${kpi('Inventario disponible',num(m.stock),`<span>Al corte: ${num(allAssets.filter(a=>a.type==='SIM'&&a.status==='Disponible').length)} SIM · ${num(allAssets.filter(a=>a.type==='eSIM'&&a.status==='Disponible').length)} eSIM</span>`,'sim')}
    ...
    <section class="panel sales-panel">${panelHeader('Evolución de ventas',`Últimos ${state.period} días del escenario · ${state.operator==='all'?'Vista consolidada':esc(S.op(state.operator).name)}`,tag('MXN'))}${chart(m.confirmed)}<div class="chart-legend"><span class="legend-key"><i></i> Ventas confirmadas</span><span>05 sep 2026 · Cierre del escenario</span></div></section>
    <section class="panel priorities-panel">${panelHeader('Prioridades operativas','Del indicador al proceso que requiere atención.')}<div class="priority-list">
      ${priority('Solicitudes de monedero',`${num(pending.length)} pendientes · ${money(pendingAmount,2)} por autorizar. Solicitudes del perfil, sin filtro de operador o periodo.`,state.role==='director'?'approvals':'wallet','wallet',pending.length?'amber':'')}
      ...
      ${priority('Continuidad de inventario',`${num(allAssets.filter(a=>a.status==='En tránsito').length)} unidades en tránsito y ${num(m.stock)} disponibles al corte. No depende del periodo.`, 'inventory','sim','','En tránsito')}
    </div><div class="operations-capacity"><div class="capacity-heading"><strong>Monedero del perfil</strong>${button('Abrir','go','arrow','link','data-page="wallet" aria-label="Abrir monedero del perfil"')}</div><div class="capacity-values"><div><span>Saldo disponible</span><strong data-capacity="balance">${money(f.balance,2)}</strong></div><div><span>Crédito disponible</span><strong data-capacity="credit">${money(Math.max(0,f.creditLimit-f.creditUsed),2)}</strong></div></div><p>Saldo actual compartido entre operadores. Las solicitudes pendientes aún no están acreditadas.</p></div></section>
    <section class="panel operator-comparison">${panelHeader('Resultado por operador','Importe, participación y operaciones confirmadas del periodo.')}
      <div class="operator-performance-list">${grouped.map(o=>{const share=m.revenue?o.value/m.revenue*100:0;return `<button type="button" class="operator-performance" data-action="universe" data-operator="${o.id}" data-revenue="${o.value}" aria-label="Explorar ${esc(o.name)}: ${esc(money(o.value,2))}, ${share.toFixed(1)}% de las ventas visibles"><span class="performance-heading"><span class="performance-name"><i class="operator-dot" style="--op:${o.color}" aria-hidden="true"></i>${esc(o.name)}</span><strong class="performance-value">${money(o.value,2)}</strong></span><span class="performance-track" aria-hidden="true"><span style="width:${share}%;--op:${o.color}"></span></span><span class="performance-detail"><span>${num(o.count)} operaciones · ${share.toFixed(1)}%</span><span>Ver universo ${icon('arrow')}</span></span></button>`;}).join('')}</div>
      <div class="operator-summary"><div><span>Ticket promedio</span><strong>${money(m.count?Math.round(m.revenue/m.count):0,2)}</strong></div><div><span>Puntos con venta</span><strong>${m.activePoints} <small>/ ${eligiblePoints.length} habilitados</small></strong></div></div>
    </section>
    <section class="panel network-card">${panelHeader('Tu red, bajo control','Puntos habilitados para el perfil y operador seleccionados.',tag('Red de prueba'))}<div class="network-numbers"><div><strong>${eligiblePoints.length}</strong><span>Puntos de venta</span></div><div><strong>${new Set(eligiblePoints.map(p=>p.distributor)).size}</strong><span>Distribuidores</span></div></div><div class="region-bars"><p class="region-caption">Participación en ventas · hasta cuatro regiones con mayor importe</p>${regionSales.slice(0,4).map(r=>`<div class="region-row"><span>${r.region}</span><div class="region-track"><span style="width:${m.revenue?r.value/m.revenue*100:0}%"></span></div><strong>${m.revenue?Math.round(r.value/m.revenue*100):0}%</strong></div>`).join('')||'<p>Sin ventas en el periodo seleccionado.</p>'}</div>${button(state.role==='pos'?'Ver mis operaciones':'Explorar red comercial','go','arrow','',`data-page="${state.role==='pos'?'sales':'network'}"`)}</section>
