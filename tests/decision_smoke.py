...
def main():
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            page.locator('#nav [data-page="overview"]').click()
            check('executive route exposes story and evaluation',page.locator('#decision-band [data-action="guide"]').count()==1 and page.locator('#decision-band [data-decision="open"]').count()==1)
            for width in [320,390,768,1024,1440]:
                page.set_viewport_size({'width':width,'height':1000})
                check(f'executive route fits {width}px',page.locator('#decision-band').evaluate('(el)=>el.scrollWidth<=el.clientWidth+1 && el.getBoundingClientRect().right<=innerWidth+1'))
            page.set_viewport_size({'width':1440,'height':1000})
            page.locator('#decision-band [data-action="guide"]').click()
            check('route opens executive story',page.locator('#experience').evaluate('(d)=>d.open'))
            page.keyboard.press('Escape')
            page.locator('#decision-band [data-decision="open"]').click()
            check('route opens evaluation without ledger changes',page.locator('#decision').evaluate('(d)=>d.open') and snapshot()==before)
            close()
            ...
            page.screenshot(path=str(OUT/'v7-evaluation-desktop.png'),animations='disabled')
            tab('proof')
            ...
            page.screenshot(path=str(OUT/'v7-evidence-desktop.png'),animations='disabled')
            tab('impact')
            ...
            page.screenshot(path=str(OUT/'v7-capacity-desktop.png'),animations='disabled')
            ...
            page.screenshot(path=str(OUT/'v7-next-step-desktop.png'),animations='disabled')
            if INLINE:
            ...
            page.screenshot(path=str(OUT/'v7-evaluation-mobile.png'),animations='disabled')
            tab('next');page.screenshot(path=str(OUT/'v7-next-step-mobile.png'),animations='disabled')
            page.keyboard.press('Escape')
            ...
            page.screenshot(path=str(OUT/'v7-failure.png'),animations='disabled');traceback.print_exc();raise
        finally:
